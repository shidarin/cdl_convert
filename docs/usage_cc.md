...
## Creating ColorCorrection

Once imported, you have two choices. You can either instantiate a new, blank CDL directly, or you can parse a file on disk.

A {class}`~cdl_convert.correction.ColorCorrection` is created with the 10 required values (RGB values for slope, offset and power, and a single value for saturation) set to their defaults.
...
:::{note}
`slope`, `offset`, `power` and `sat` are convenience properties that actually reference two child objects of {class}`~cdl_convert.correction.ColorCorrection`: a {class}`~cdl_convert.correction.SopNode` and a {class}`~cdl_convert.correction.SatNode`. Calling them via `cc.power` is the same as calling `cc.sop_node.power`.
:::

The {class}`~cdl_convert.correction.ColorCorrection` class inherits from both the {class}`~cdl_convert.base.AscColorSpaceBase` class and the {class}`~cdl_convert.base.AscDescBase` class, giving it the additional attributes of `input_desc` (to describe the colorspace entering the correction), `viewing_desc` (to describe the colorspace conversions that must occur for viewing, and what type of monitor was used), and `desc` (which can be an infinitely long list of shot descriptions).

### Direct Creation

If you want to create a new instance of {class}`~cdl_convert.correction.ColorCorrection`, you have to provide an `id` for the unique CDL identifier and an optional source filename to `input_file`.
...
Instead of creating a blank CDL object, you can parse a `cc` file from disk, and it will return a single {class}`~cdl_convert.correction.ColorCorrection` matching the correction found in the file. Formats that contain multiple corrections will return a {class}`~cdl_convert.collection.ColorCollection`, which contains child {class}`~cdl_convert.correction.ColorCorrection` objects.

If you don't want to worry about matching the filetype to a parser, just use the generic {func}`~cdl_convert.parse.parse_file` function.
...
Once you have a {class}`~cdl_convert.correction.ColorCorrection` from a parser, you'll find that whatever values it found in the file now exist on the instance of {class}`~cdl_convert.correction.ColorCorrection`.
...
:::{note}
When parsing, the `id` attribute is set in a variety of ways depending on how much information is available. Some formats, like `cc`, have an explicitly tagged `id` field that is always used. Other formats, like `flex`, have no such field and the parser tries to grab any scene/take metadata it can find to construct one. The last fallback is always the filename. For formats that can contain multiple {class}`~cdl_convert.correction.ColorCorrection` objects, the `id` has a created instance number after it.
:::
...
Setting the CDL slope, offset and power (SOP) values is as easy as passing them any list or tuple with three values. Integers, strings and floats will be automatically converted to Decimals, while slope and power will also truncate at zero (negative values are not allowed).
...
Saturation is a positive float value, and the same checks and conversions that we do on SOP values happen for saturation as well.
...
You can append to the list by setting the description field like normal.
...
When creating a {class}`~cdl_convert.correction.ColorCorrection`, the `id` field is checked against a global list of {class}`~cdl_convert.correction.ColorCorrection` ids, and creation fails if the `id` is not unique.
...
At the current time, `file_out` cannot be set directly. `file_out` is determined by using the class method `determine_dest`, which takes a provided directory, the `id` and determines the output path.
...
## Writing ColorCorrection

When you're done tinkering with the {class}`~cdl_convert.correction.ColorCorrection` instance, you might want to write it out to a file. We need to give {class}`~cdl_convert.correction.ColorCorrection` the file extension we plan to write to, then call a `write` function with our {class}`~cdl_convert.correction.ColorCorrection` instance, which will actually convert the values on the {class}`~cdl_convert.correction.ColorCorrection` into the format desired, then write that format to disk.
...
See {doc}`api/write` for all available write functions.

:::{warning}
It is highly likely that in the future, these will be methods on the {class}`~cdl_convert.correction.ColorCorrection` class itself, and that instead of writing the file directly, they will instead return a string formatted for writing.
:::

## Working with ColorValues (New in 1.0)

Version 1.0 introduces the `ColorValues` dataclass for structured color correction data with validation.

### Getting Color Values

You can retrieve all color values as a structured dataclass:

```python
>>> values = cc.get_color_values()
>>> print(values.slope)
(Decimal('1.2'), Decimal('1.1'), Decimal('1.0'))
>>> print(values.saturation)
Decimal('0.9')
```

The pre-existing methods and properties function as before.

### Checking for Unity

The `ColorValues` dataclass provides a convenient method to check if values are at unity (no correction applied):

```python
>>> values = cc.get_color_values()
>>> print(values.is_unity())
False  # Because we have non-unity values
```

### Creating ColorValues Directly

You can also create `ColorValues` instances directly for validation:

```python
>>> from cdl_convert import ColorValues
...
The `ColorValues` dataclass automatically validates values during initialization, raising `ValidationError` if values are invalid.
