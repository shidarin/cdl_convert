...
class TestParseOTIO(unittest.TestCase):
    ...
    def test_parse_otio_successful_single_clip(self):
        ...
        self.assertEqual(cc.slope, (Decimal('1.2'), Decimal('1.1'), Decimal('1.0')))
        self.assertEqual(cc.offset, (Decimal('-0.1'), Decimal('0.0'), Decimal('0.1')))
        self.assertEqual(cc.power, (Decimal('0.9'), Decimal('1.0'), Decimal('1.1')))
    ...
    def test_parse_otio_malformed_file_error(self):
        ...
        mock_otio = mock.MagicMock()
        mock_otio.adapters.available_adapter_names.return_value = ['otio_json']
        mock_otio.adapters.read_from_file.side_effect = Exception("Invalid JSON format")
    ...
    def test_parse_otio_otio_json_adapter_missing(self):
        ...
        mock_otio = mock.MagicMock()
        mock_otio.adapters.available_adapter_names.return_value = ['other_adapter']
        ...
        with mock.patch.dict('sys.modules', {'opentimelineio': mock_otio}):
            ...
            error_msg = str(cm.exception)
            self.assertIn("Built-in OTIO adapter 'otio_json' not available", error_msg)
    ...
    def test_parse_otio_clips_with_partial_cdl_data(self):
        ...
        # Create mock track and timeline
        mock_track = [mock_clip_sop_only, mock_clip_sat_only, mock_clip_incomplete_sop]
        mock_timeline = mock.MagicMock()
        mock_timeline.tracks = [mock_track]
