...
class TestParseFLExBasic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.power1 = decimalize(0.993, 0.998, 1.0113)
    ...
    def testCollection(self):
        ...
        self.assertEqual(cdl_convert.ColorCollection, self.cdls.__class__)

    # ==========================================================================

    def testFileIn(self):
        """Tests that file_in has been set on the collection correctly"""
        self.assertEqual(Path(self.filename).resolve(), self.cdls.file_in)

    # ==========================================================================

    def testType(self):
        """Test that the type of the collection is set to ccc"""
        self.assertEqual("ccc", self.cdls.type)

    # ==========================================================================
    def testId(self):
        """Tests that filenames were parsed correctly"""

        self.assertEqual("bb94_x103_line1", self.cdl1.id)

        self.assertEqual("bb94_x104_line2", self.cdl2.id)

        self.assertEqual("bb94_x105_line3", self.cdl3.id)
    ...
    def testSlope(self):
        ...
        self.assertEqual(self.slope1, self.cdl1.slope)

        self.assertEqual(self.slope2, self.cdl2.slope)

        self.assertEqual(self.slope3, self.cdl3.slope)
    ...
    def testOffset(self):
        ...
        self.assertEqual(self.offset1, self.cdl1.offset)

        self.assertEqual(self.offset2, self.cdl2.offset)

        self.assertEqual(self.offset3, self.cdl3.offset)
    ...
    def testPower(self):
        ...
        self.assertEqual(self.power1, self.cdl1.power)

        self.assertEqual(self.power2, self.cdl2.power)

        self.assertEqual(self.power3, self.cdl3.power)
    ...
    def testSat(self):
        ...
        self.assertEqual(self.sat1, self.cdl1.sat)

        self.assertEqual(self.sat2, self.cdl2.sat)

        self.assertEqual(self.sat3, self.cdl3.sat)
...
class TestParseFLExMissingNames(TestParseFLExBasic):
    ...
    def setUp(self):
        ...
        line2 = buildFLExTake(
            self.slope2, self.offset2, self.power2, self.sat2, "bb94", "x104"
        )
        ...
        line3 = buildFLExTake(
            self.slope3, self.offset3, self.power3, self.sat3, "bb94"
        )
    ...
    def testId(self):
        ...
        self.assertEqual("bb94_x104", self.cdl2.id)

        self.assertEqual("bb94", self.cdl3.id)
...
class TestParseFLExNoTitle(TestParseFLExBasic):
    ...
    def testId(self):
        ...
        self.assertEqual(f"{filename}001", self.cdl1.id)

        self.assertEqual(f"{filename}002", self.cdl2.id)

        self.assertEqual(f"{filename}003", self.cdl3.id)
    ...
    def testDescription(self):
        ...
        for i in range(3):
            self.assertEqual([], self.cdls.all_children[i].desc)
...
class TestParseFLExMissingSopSat(TestParseFLExBasic):
    ...
    def setUp(self):
        ...
        line1 = buildFLExTake(
            sat=self.sat1, scene="bb94", take="x103", roll="line1"
        )
    ...
    def testCollection(self):
        """Tests that we were returned a ColorCollection"""
        self.assertEqual(cdl_convert.ColorCollection, self.raw_cdls.__class__)

    # ==========================================================================

    def testFileIn(self):
        """Tests that file_in has been set on the collection correctly"""
        self.assertEqual(Path(self.filename).resolve(), self.raw_cdls.file_in)

    # ==========================================================================

    def testType(self):
        """Test that the type of the collection is set to ccc"""
        self.assertEqual("ccc", self.raw_cdls.type)
    ...
    def testOnlyTwoCDLsReturned(self):
        ...
        self.assertEqual(2, len(self.raw_cdls.all_children))
...
def buildFLExTake(
    ...
    if sat:
        flex += FLEX_702.format(sat=str(sat).ljust(6, " ")[:6])
